...
if ffi_ok then
    ...
    local function resolve_game_steam()
        if _steam_user_ptr then return true end
    ...
    function steam.cancel_auth_ticket(handle)
        if not _cancel_auth_ticket_fn or not _steam_user_ptr or not handle then return end
        pcall(function()
    ...
    function steam.get_auth_ticket() return nil, "FFI not available for auth tickets" end
